...
export class SseClient {
  ...
  async connectGet(url: string, onEvent: (event: SseEvent) => void, onError?: (err: any) => void) {
    ...
    await this.connectGetOnce(url, onEvent, onError);
  }

  private async connectGetOnce(url: string, onEvent: (event: SseEvent) => void, onError?: (err: any) => void) {
    this.abortController = new AbortController();
    try {
      const response = await fetch(url, {
        method: 'GET',
      ...
      if (err.name !== 'AbortError' && !this.manualClosed) {
        onError?.(err);
        this.scheduleReconnect(url, onEvent, onError);
      }
    }
  }

  /** 指数退避重连：1s → 2s → 4s → 8s → 16s */
  private scheduleReconnect(url: string, onEvent: (event: SseEvent) => void, onError?: (err: any) => void) {
    ...
    this.reconnectTimer = setTimeout(() => {
      this.connectGetOnce(url, onEvent, onError);
    }, delay);
  }
